...
def hunt(player_status, hours_spent):
    ...
    food_gained = 10 * math.sqrt(hours_spent) * random.uniform(0.9, 1.1)
...
def rest(player_status, hours_spent):
    aging_cost = hours_spent * (random.randint(1, player_status['age'] // 5 + 1) + random.uniform(0, 1))
    health_gained = 8 * math.sqrt(hours_spent) * random.uniform(0.9, 1.1)
    food_lost = (hours_spent ** 1.25) * random.uniform(0.75, 2)
